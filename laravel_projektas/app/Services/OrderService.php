...
class OrderService
{
    ...
    // cia is session krepselio sukuriamas tikras uzsakymas DB.
    public function createFromCart(array $customer, ?string $paymentMethod = null): Order
    {
        // uzsakymui vartotojas turi buti prisijunges.
        if (!Auth::check()) {
        ...
        // cia paimamas tas pats krepselis, kuris buvo saugomas session.
        $cart = $this->cart->get();
        ...
        // transakcija uztikrina, kad orderis, prekes ir mokejimas susikurtu kartu.
        $order = DB::transaction(function () use ($cart, $customer, $paymentMethod) {
            ...
            // cia kiekviena krepselio preke perkeliama i order_items lentele.
            foreach ($cart as $item) {
                ...
                // custom_tub yra individualus kubilas is builderio, o ne paprasta katalogo preke.
                if ($type === 'custom_tub') {
            ...
            // sukuriamas mokejimo irasas, pradzioje jis dar neapmoketas.
            Payment::create([
                'order_id' => $order->id,
